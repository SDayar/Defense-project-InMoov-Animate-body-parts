...
///@brief lle exécute les positions des doigts et pas celle du poignet. 
  ...
  ///@return void
    ...
    for(JsonArray sub_tab_pos : s->positions_buffer){
      where_am_i("Execution de sous-positions "+String(sub_tab_pos));
       move_finger_to_position(0, sub_tab_pos[0]);
       move_finger_to_position(1, sub_tab_pos[1]);
      move_finger_to_position(2, sub_tab_pos[2]);
       move_finger_to_position(3, sub_tab_pos[3]);
        move_finger_to_position(4, sub_tab_pos[4]);
